...
from deasc import TuningDyn_Turbines_CI
...
Dynamic parameter tuning with column independence (CI) is introduced in the optimisation
for the wake expansion parameter k of the Jensen wake model. The tuning variables are the
yaw angles of the turbines in the two most upstream rows.
"""
...
wf_model.set_aligned_layout(3, 3, 7, 5)
...
yaw_initial = np.full(shape=(9), fill_value=0)
...
tuning_dyn_obj = TuningDyn_Turbines_CI(param_class=parameter_class,
                                       param_name=parameter_name,
                                       turbines_cols=[[1, 4, 7], [2, 5, 8], [3, 6, 9]],
                                       tuning_turbines_cols_dict={'3x1': [1, 2]},
                                       GP_model_cols_dict={'3x1': GP_model})
